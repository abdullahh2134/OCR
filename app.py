...
def extract_patient_data(full_text):
    ...
    patterns = {
        'Age': r"(?:Age|Patient Age)\s*:?\s*(\d+)\s*(?:years)?",
         'Gender': r"(?:Gender|Sex)\s*:?\s*(Male|Female|M|F|\w+)",
        ...
        'Total Bilirubin': r"(?:Total Bilirubin|TBIL)\s*:?\s*([\d.]+)\s*(?:mg/dL)?",
        'Direct Bilirubin': r"(?:Direct Bilirubin|DBIL)\s*:?\s*([\d.]+)\s*(?:mg/dL)?",
        'Alkaline Phosphotase': r"(?:Alkaline Phosphotase|ALP|Alkaline Phosphatase)\s*:?\s*(\d+)\s*(?:U/L)?",
        'Sgpt': r"(?:Sgpt|ALT|Alanine Aminotransferase)\s*:?\s*(\d+)\s*(?:U/L)?",
        'Sgot': r"(?:Sgot|AST|Aspartate Aminotransferase)\s*:?\s*(\d+)\s*(?:U/L)?",
         'Total Proteins': r"(?:Total Proteins|Protein Total)\s*:?\s*([\d.]+)\s*(?:g/dL)?",
        'Albumin_G': r"(?:Albumin_G|Albumin \(AL\)|ALB|Albumin)\s*:?\s*([\d.]+)\s*(?:g/dL)?",
         'A/G Ratio': r"(?:A/G Ratio|Albumin/Globulin Ratio)\s*:?\s*([\d.]+)",
